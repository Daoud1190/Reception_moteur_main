...
void loop() {
  ...
  // Vérifie si des données sont disponibles sur le Bluetooth
  if (SerialBT.available()) {
    char caracter_recu = SerialBT.read();  // Lire le caractère envoyé via Bluetooth
    receiveData[0] = caracter_recu;

    // Interpréter le caractère reçu pour déterminer le mode sélectionné
    if (receiveData[0] == '&') {
      // Parcourt les données restantes
      ...
      if (receiveData[6] == (receiveData[1] ^ receiveData[2] ^ receiveData[3] ^ receiveData[4] ^ receiveData[5]))
    ...
    if (receiveData[0] == '@') {
      ...
      // Définit les positions des servomoteurs en fonction des encodeurs
...
void pouce() {
  ...
  mappedValue = map(255 - encoderValue, 0, 100, 50, 100);
